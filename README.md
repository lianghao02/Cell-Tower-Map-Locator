...
專為執法人員、刑事鑑識與救難人員打造的現代化基地台定位與空間關聯分析工具。支援各大電信簡訊／調閱文字自動智慧解析、方位角清透扇形繪製、多點動態時序軌跡、自身 GPS 戰術導航、**最近歷史多筆疊加比對**，以及**🎯 扇形交集空間分析引擎 (自動求交集熱區與覆蓋面積)**。
...
- **警戒紅高亮圖層**：於地圖繪製警戒亮紅（fillOpacity 0.48）多邊形與「🎯 訊號交集核心熱點」Marker。
- **空間度量指標**：即時輸出交集覆蓋面積（$m^2$ / 公頃）與中心點經緯度，並提供「📍 聚焦熱區」與「👁️ 僅看交集」視角切換。
- **判讀邊界**：交集區與其幾何中心僅為依基地台座標、方位角及目前扇區設定計算的分析參考，不代表手機實際或精確位置。

### 2. 🧭 方位角扇形即時繪製引擎
- **手動自訂角度**：在「定位資料」頁籤輸入 0~360° 方位角，地圖即時以正北為基準順時針繪製寶藍色 15% 晶透扇形。
- **波束與半徑微調**：可至右上角進階設定調整扇形涵蓋半徑（50~1000m）與開口夾角（10~180°）。

### 3. 🗂️ 最近歷史「多筆疊加空間比對 (Multi-History Comparison)」
- **同屏多色疊加**：在「最近歷史」頁籤勾選 2 筆以上紀錄，地圖自動為各歷史項目分配高辨識度專屬對比色（#1 寶藍、#2 翡翠綠、#3 琥珀橘、#4 紫羅蘭、#5 玫瑰紅等）。
- **全景自適應視野**：自動計算所有勾選點位的經緯度邊界（FitBounds），一次盡收眼底。

### 4. 🗺️ 五大電信格式智慧適應器 (Regex Auto-Parser)
